...
- **`/requirements.txt: not found`** in Docker build → the Git deploy did not contain `requirements.txt` at the service **Root Directory**, or it was never committed. The Dockerfile installs the same packages without that file; redeploy after pulling the updated `Dockerfile`. If you customize deps, mirror them in **both** `requirements.txt` and the `pip install` line in `Dockerfile`.
