"""
Goatys CC — Telegram shop bot (balance, BIN stock, BTC/LTC top-up).
"""
...
# Serialize bin_stock loads/saves — prevents duplicate lines + stale counts under overlap.
_stock_io_lock = asyncio.Lock()
...
# Default on-chain deposit addresses (override via BTC_WALLET / LTC_WALLET in .env).
_DEFAULT_SHOP_BTC = "bc1qvykus2xjawptxwz7qp0mwn5zxdetvzydv40eml"
_DEFAULT_SHOP_LTC = "ltc1qugzzl82j354mr69f3qtrm2fshhczpmxvh2vxrn"

WALLET_BTC = _env_wallet(
    "BTC_WALLET",
    _env_wallet(
        "BASE_MONEYJR_BTC",
        _env_wallet("BASE_GTJM_BTC", _DEFAULT_SHOP_BTC),
    ),
).strip()
WALLET_LTC = _env_wallet(
    ...
    _env_wallet(
        "BASE_MONEYJR_LTC",
        _env_wallet("BASE_GTJM_LTC", _DEFAULT_SHOP_LTC),
    ),
).strip()
...
def save_bin_stock() -> None:
    ...
    try:
        ...
        log.error(
            "Could not write bin_stock.json — stock may NOT persist: %s",
            e,
        )
...
def parse_bulk_stock_opening_message(text: str) -> tuple[float, list[str]] | None:
    """
    Leading line must be ``/stock $price``. Rest of message = optional pipe dumps.
    Returns (price, raw_card_chunks) — chunks may be empty (price-only starter).
    None if not a bulk opener (wizard /stock, /stock bin, etc.).
    """
    ...
    return price, raw_cards


def merge_bulk_pipe_raw_into(
    by_bin: dict[str, list[str]],
    errors: list[str],
    raw_cards: list[str],
    line_no_start: int,
) -> int:
    """Append validated pipe dumps into buckets; return next line index for diagnostics."""
    i = line_no_start
    for raw in raw_cards:
        rs = (raw or "").strip()
        if not rs:
            continue
        line, bin6, err = parse_bulk_pipe_line(rs)
        if err:
...
def bulk_collect_running_summary_html(bc: dict[str, Any]) -> str:
    price = float(bc.get("price") or 0.0)
    by_bin = bc.get("by_bin") or {}
    errors = list(bc.get("errors") or [])
    tot = sum(len(v) for v in by_bin.values())
    bins_n = len(by_bin)
    parts: list[str] = [
        "📦 <b>Bulk import</b> — <i>listening for more pasted lines</i>\n\n",
        f"Locked price: <b>{fmt_usd(price)}</b> per line\n",
        f"Buffered: <b>{bins_n}</b> BIN bucket(s) · <b>{tot}</b> valid line(s).\n",
    ]
    if errors:
        parts.append(
            f"\n⚠️ <b>Skipped so far:</b> <code>{len(errors)}</code> trouble line(s).\n"
        )
        for err in errors[:10]:
            parts.append(f"· <code>{html.escape(err[:200])}</code>\n")
        if len(errors) > 10:
            parts.append(f"<i>… +{len(errors) - 10} more skip notes</i>\n")
    parts.append(
        "\n<i>Long lists may arrive as multiple Telegram messages — paste every part here.\n"
        "When you are finished, send</i> <code>/done</code> <i>to review BIN counts and "
        "pick Live / Unchecked piles.\n<code>/cancelstock</code> aborts.</i>"
    )
    return "".join(parts)
...
def clear_admin_stock_flow(
    ...
    if user_id is not None:
        ...
        _bulk_collect_user_ids.discard(int(user_id))
...
def shop_line_pick_crc(line: str) -> str:
    """Short checksum so callback identifies the exact stocked line."""
    return "%08x" % (zlib.crc32(line.encode("utf-8", errors="replace")) & 0xFFFFFFFF)


def lines_sorted_for_section(sec: dict[str, Any]) -> list[str]:
    raw = list(sec.get("lines") or [])
    filtered, _, _ = filter_lines_for_buy_catalog(raw, 0, FILTER_KIND_ALL, 0)
    out = list(filtered)
    sort_stock_lines(out)
    return out


def format_shop_line_pick_button_text(line: str) -> str:
    bk = extract_bin_prefix_from_line(line) or "?"
    city, st = extract_city_state_from_line(line)
    txt = f"{bk} {city}, {st}".replace("\n", " ").strip()
    if len(txt) <= BIN_BTN_TEXT_MAX:
        return txt
    return txt[: BIN_BTN_TEXT_MAX - 1] + "…"
...
# After “Custom” on top-up keypad — next text message is interpreted as USD amount (min $10).
_awaiting_custom_topup_user_ids: set[int] = set()


def mark_awaiting_custom_topup(user_id: int) -> None:
    _awaiting_custom_topup_user_ids.add(int(user_id))


def clear_awaiting_custom_topup(user_id: int) -> None:
    _awaiting_custom_topup_user_ids.discard(int(user_id))


class _AwaitingCustomTopupMessageFilter(filters.MessageFilter):
    """After Custom amount — next plain text is interpreted as USD (min $10)."""
    ...
    def filter(self, message: Message) -> bool:
        ...
        if uid in _admin_stock_wizard_user_ids:
            return False
        if uid in _bulk_collect_user_ids:
            return False
        return uid in _awaiting_custom_topup_user_ids


AWAITING_CUSTOM_TOPUP_FILTER = (
    filters.TEXT
    & filters.ChatType.PRIVATE
    & _AwaitingCustomTopupMessageFilter()
)
...
class _AwaitingTxLinkMessageFilter(filters.MessageFilter):
    ...
    def filter(self, message: Message) -> bool:
        ...
        if message.text.strip().startswith("/"):
            return False
        uid = message.from_user.id
        if uid in _awaiting_custom_topup_user_ids:
            return False
        return uid in _awaiting_tx_link_user_ids
...
# Admin “bulk /stock $price …” accumulator (until /done picks piles).
_bulk_collect_user_ids: set[int] = set()
...
class _AdminBulkCollectMessageFilter(filters.MessageFilter):
    """Admin pasting continuation chunks for ``/stock $price`` bulk capture."""
    ...
    def filter(self, message: Message) -> bool:
        ...
        if message.text.strip().startswith("/"):
            return False
        uid = message.from_user.id
        if uid not in _bulk_collect_user_ids:
            return False
        return is_admin(uid)


ADMIN_BULK_COLLECT_FILTER = (
    filters.TEXT
    & filters.ChatType.PRIVATE
    & _AdminBulkCollectMessageFilter()
)
...
def parse_custom_topup_usd(text: str) -> float | None:
    """Parse a lone USD amount from user text (minimum apply in caller)."""
    raw = (text or "").strip()
    if not raw:
        return None
    s = raw[1:].strip() if raw.startswith("$") else raw
    s = re.sub(r"(?i)\s*usd\s*$", "", s).strip()
    s = s.replace(",", "").replace(" ", "")
    if s.count(".") > 1:
        return None
    try:
    ...
    if v != v or v <= 0:  # NaN
        return None
    return round(min(v, 1_000_000.0), 2)


def credits_reply_label(user_id: int) -> str:
    u = ensure_user(user_id)
    t = user_shop_balance(u)
    if t == int(t):
        return f"Credits: {int(t)}"
    return f"Credits: {t:.2f}"


def shop_buy_intro_inline() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("📋 Browse BINs", callback_data="shop_browse_bins")],
            [InlineKeyboardButton("ℹ️ How buying works", callback_data="shop_how_buy")],
        ]
    )
...
def section_catalog_text_and_keyboard(
    ...
    """List every stocked pipe line — BIN plus city/state. Tap buys that line instantly."""
    bank_i = 0
    kind_k = FILTER_KIND_ALL
    level_i = 0
    ...
    price = float(sec.get("price_usd", 0.0))
    lines_sorted = lines_sorted_for_section(sec)
    total_lines = len(lines_sorted)
    total_raw = len(list(sec.get("lines") or []))

    if total_lines == 0:
        empty_hint = (
            "<i>This section has no stocked lines.</i>"
            if total_raw == 0
            else "<i>No usable pipe lines.</i> Ask an admin to check stock format."
        )
        text = (
            f"💳 <b>{head}</b>\n"
    ...
    chunk = lines_sorted[start : start + BUY_CATALOG_PAGE_SIZE]
    more_note = ""
    if total_lines > BUY_CATALOG_PAGE_SIZE:
        more_note = (
            f"\n<i>Lines {start + 1}–{start + len(chunk)} "
            f"of {total_lines}</i>"
        )
    ...
    text = (
        f"💳 <b>{head}</b>\n"
        f"Price: <b>{fmt_usd(price)}</b> each · "
        f"<b>In stock:</b> {total_lines}"
        + ("" if total_lines == total_raw else f" <i>({total_raw} raw)</i>")
        + "\n"
        f"<b>Balance:</b> {fmt_usd(bal)}\n"
        f"{notes_block}\n"
        "Tap any row to <b>buy that line instantly</b> · balance drops per tap · "
        "the listing refreshes here so you can keep shopping."
        f"{more_note}"
    )

    rows: list[list[InlineKeyboardButton]] = []
    for off, ln in enumerate(chunk):
        abs_idx = start + off
        crc = shop_line_pick_crc(ln)
        cb = f"cac|{slug}|{abs_idx}|{crc}|{page}"
        rows.append(
            [
                InlineKeyboardButton(
                    format_shop_line_pick_button_text(ln),
                    callback_data=cb,
                )
            ]
        )
    ...
    next_cb = (
        pack_browse_cb(slug, page + 1, bank_i, kind_k, level_i)
        if start + BUY_CATALOG_PAGE_SIZE < total_lines
        else None
    )
    if prev_cb:
...
def try_take_shop_line_by_pick_unlocked(
    sid: str, pick_idx: int, pick_crc: str
) -> str | None:
    """Pop the stocked line at sorted-catalog index with matching CRC."""
    sec = BIN_SECTIONS.get(sid)
    if not sec:
        return None
    want_crc = pick_crc.strip().lower()
    sorted_view = lines_sorted_for_section(sec)
    if pick_idx < 0 or pick_idx >= len(sorted_view):
        return None
    target = sorted_view[pick_idx]
    if shop_line_pick_crc(target) != want_crc:
        return None
    nth = sum(1 for i in range(pick_idx) if sorted_view[i] == target)
    raw = list(sec.get("lines") or [])
    seen = 0
    rip: int | None = None
    ...
    took = raw.pop(rip)
    if not raw:
        BIN_SECTIONS.pop(sid, None)
    else:
        sec["lines"] = raw
    return took


async def execute_shop_sale_pick(
    user: User,
    slug: str,
    pick_idx: int,
    pick_crc: str,
    *,
    record_order_row: bool = True,
) -> tuple[str | None, str | None, str, float]:
    """Buy the exact line at catalog index (checksum must match)."""
    async with _stock_io_lock:
        load_bin_stock()
        sid = slug_to_section_id(slug.strip())
        if not sid or sid not in BIN_SECTIONS:
            return None, sid or "", "section", 0.0
        sec = BIN_SECTIONS[sid]
        price = float(sec.get("price_usd", 0.0))
        u = ensure_user(user.id)
        if user_shop_balance(u) < price:
            return None, sid, "balance", price
        took = try_take_shop_line_by_pick_unlocked(sid, pick_idx, pick_crc)
        if not took:
            return None, sid, "stock", price
        save_bin_stock()
        u["balance"] = float(u.get("balance", 0.0)) - price
        u["spent"] = float(u.get("spent", 0.0)) + price
        bk = extract_bin_prefix_from_line(took) or "?"
        if record_order_row:
            try:
                record_purchase_order(user.id, "shop", sid, bk[:6], price, took)
            except Exception:
                log.warning("orders.json write failed uid=%s", user.id, exc_info=True)
        return took, sid, "", price
...
async def handle_main_reply_menu(
    ...
    if uid in _awaiting_custom_topup_user_ids:
        clear_awaiting_custom_topup(uid)
    ...
    if text == BTN_REPLY_BUY:
        await msg.reply_text(
            "💳 <b>Buy cards</b>\n\n"
            "Open a <b>pile</b> (live / unchecked). Every row shows "
            "<b>BIN + City, ST</b> for one stocked card.\n"
            "Tap any row → <b>instant purchase</b> (balance deducted, line delivered "
            "below); the listing refreshes in place so you can keep buying.\n\n"
            "Use <b>🔍 Filters</b> on the keypad to jump back to the section list anytime.",
            reply_markup=shop_buy_intro_inline(),
            parse_mode="HTML",
        )
        raise ApplicationHandlerStop

    if text == BTN_REPLY_FILTERS:
        await msg.reply_text(
            "🔍 <b>Browse BIN listings</b>\n\n"
            "Sections are BIN × pile · inside, each stocked line lists as BIN + City, ST.",
    ...
    if text == BTN_REPLY_ORDERS:
        ...
        raise ApplicationHandlerStop
    if text.startswith("Credits:") or text.startswith("Balance:"):
        u = ensure_user(uid)
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "adm_port_upload":
        ...
        await query.message.reply_text(
            ...
            "<b>Telegram often splits long pastes into several messages</b> — send every chunk, "
            "then <code>/done</code> to review. The bot groups by PAN BIN — tap "
            "<b>Live pile</b> or <b>Unchecked pile</b> to publish.\n\n"
    ...
    if data.startswith("bpr:"):
        ensure_user(user.id)
        await query.answer(
            "That button is from an old listing. Open BINs again.", show_alert=True
        )
        return
    ...
    if data.startswith("bca:"):
        ensure_user(user.id)
        await query.answer(
            "Old BIN menu — go back and open the pile again.", show_alert=True
        )
        return

    if data.startswith("cac|"):
        ensure_user(user.id)
        parts = data.split("|")
        if len(parts) != 5 or parts[0] != "cac":
            await query.answer(
                "Stale listing — reopen the pile from 📋 Browse BINs.",
                show_alert=True,
            )
            return
        slug_raw = parts[1].strip()
        crc_hex = parts[3].strip().lower()
        try:
            pick_idx = int(parts[2].strip())
            cur_page = int(parts[4].strip())
        except ValueError:
            await query.answer("Bad row id.", show_alert=True)
            return
        if pick_idx < 0 or len(crc_hex) != 8:
            await query.answer("Bad row.", show_alert=True)
            return

        kb_page = max(0, cur_page)

        async def refresh_catalog() -> None:
            nt, kb = section_catalog_text_and_keyboard(
                slug_raw, user.id, page=kb_page
            )
            await edit_safe(query, nt, kb)

        line, sid, err, paid = await execute_shop_sale_pick(
            user, slug_raw, pick_idx, crc_hex
        )
        ...
        if err == "balance":
            ...
            await query.answer(
                f"Insufficient balance · have {fmt_usd(have)} · need {fmt_usd(paid)}.",
                show_alert=True,
            )
            await refresh_catalog()
            return
        if err == "section" or err == "stock" or not line or not sid:
            await query.answer(
                "That line sold or the listing moved — refreshed.",
                show_alert=False,
            )
            await refresh_catalog()
            return
        ...
        bk_show = html.escape(extract_bin_prefix_from_line(line) or "?")
        await query.message.reply_text(
            "✅ <b>Delivered</b> · "
            f"{section_catalog_html_title(sid)}\n"
            f"BIN <code>{bk_show}</code> · paid {fmt_usd(paid)}\n\n"
    ...
    if data == "cart_go":
        ensure_user(user.id)
        await query.answer(
            "Checkout is disabled — tap a row on the BIN list to buy instantly.",
            show_alert=True,
        )
        await query.message.reply_text(
            "💳 Tap <b>📋 Browse BINs</b>, open a pile, then tap any row to charge "
            "and receive that line.",
            reply_markup=shop_buy_intro_inline(),
            parse_mode="HTML",
        )
        return
    ...
    if data == "cart_clr":
        ensure_user(user.id)
        await query.answer("No cart anymore — buys are tap-to-purchase.", show_alert=False)
        return

    if data == "shop_browse_bins":
        ensure_user(user.id)
        await query.answer()
        await query.message.reply_text(
            "💳 <b>Select a BIN section</b>",
    ...
    if data == "shop_how_buy":
        ensure_user(user.id)
        await query.answer()
        await query.message.reply_text(
            "ℹ️ <b>How buying works</b>\n\n"
            "• Tap a BIN <b>section/pile</b> from the picker.\n"
            "• Every line shows <b>BIN + City, ST</b> · tap charges your balance "
            "and ships that row immediately.\n"
            "• The pile message updates in place — keep tapping more rows anytime.\n"
            "• Top up credits first if balance is below the listed price.",
            parse_mode="HTML",
        )
        return
    ...
    if data == "tu_custom":
        mark_awaiting_custom_topup(user.id)
        await edit_safe(
            query,
            "💰 <b>Custom top-up amount</b>\n\n"
            "Minimum deposit: <b>$10.00</b> USD.\n\n"
            "<b>Send one message</b> with just the amount, for example "
            "<code>25</code> or <code>$100.50</code>.",
            InlineKeyboardMarkup(
    ...
    if data == "tu_restart":
        clear_awaiting_custom_topup(user.id)
    ...
    if data == "m_bases":
        await query.message.reply_text(
            "🏦 <b>Payments</b> route to <code>BTC_WALLET</code> and "
            "<code>LTC_WALLET</code> (with fallbacks from older <code>BASE_*</code> vars).",
            parse_mode="HTML",
        )
        return
    ...
    if data in ("m_cart", "m_buy"):
        await query.message.reply_text(
            "💳 <b>Buy cards</b>\n\n"
            "Pick a pile — BIN + City, ST per row; tap a row for instant delivery.",
            reply_markup=buy_menu_keyboard(),
            parse_mode="HTML",
        )
        return
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    user_lines = (
        ...
        "/start — menu & keypad\n"
        "/help — this list\n"
        "/myid — your Telegram user id (for granting admin)\n"
        "/cancel — stop custom top-up or TX-link entry, or an admin stock flow\n\n"
        "<b>Keypad</b>\n"
        "• <b>Top-up Balance</b> — preset amounts or <b>Custom</b> (min $10), "
        "then BTC or LTC\n"
        "• <b>Buy cards</b> — BIN pile listing · tap a row · instant delivery\n"
        "• <b>Filters</b> — jump to BIN section list\n"
        "• <b>Credits:</b> / <b>Orders</b> — balance & purchase history"
    )
    if not is_admin(uid):
        await update.message.reply_text(
            "<b>Goatys — how to use</b>\n\n" + user_lines,
            parse_mode="HTML",
        )
        return
    admin_block = (
        "\n\n<b>Admin</b>\n"
        "/admin — portal · /users — roster\n"
        "/announce … — DM all known users "
        "(or use 📢 from the portal)\n"
        "/stock … — bulk paste · wizard · "
        "<code>/stock bin BIN $price PAN MM/YY CVV</code> (live pile)\n"
        "/done — commit wizard buffer · "
        "<code>/cancelstock</code> exits stock upload\n"
        "/payportal · /pendingclaims · /allclaims · "
        "/accept &lt;id&gt; · /reject &lt;id&gt; — payment claims"
    )
    await update.message.reply_text(
        "<b>Goatys — how to use</b>\n\n" + user_lines + admin_block,
        parse_mode="HTML",
    )


MIN_CUSTOM_TOPUP_USD = 10.0


async def handle_custom_topup_message(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    """Interpret the next DM as USD amount after tapping Custom on the top-up keypad."""
    msg = update.effective_message
    user = update.effective_user
    if not msg or not user or not msg.text:
        return
    uid = user.id
    if uid not in _awaiting_custom_topup_user_ids:
        return
    parsed = parse_custom_topup_usd(msg.text)
    if parsed is None:
        await msg.reply_text(
            "Could not read that amount. Send one number, e.g. <code>25</code>, "
            "<code>$100.50</code>, or <code>1,000</code>. Minimum "
            f"<b>{fmt_usd(MIN_CUSTOM_TOPUP_USD)}</b>.",
            parse_mode="HTML",
        )
        return
    if parsed + 1e-9 < MIN_CUSTOM_TOPUP_USD:
        await msg.reply_text(
            f"Minimum deposit is <b>{fmt_usd(MIN_CUSTOM_TOPUP_USD)}</b>. "
            "Try a larger amount.",
            parse_mode="HTML",
        )
        return
    clear_awaiting_custom_topup(uid)
    context.user_data["pending_invoice"] = parsed
    context.user_data["pay_source"] = "topup"
    context.user_data.pop("pay_coin", None)
    clear_awaiting_tx_link(uid)
    context.user_data.pop("awaiting_tx_link", None)
...
async def handle_bulk_stock_collect_message(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    """Continuation messages after ``/stock $price`` (Telegram-split long dumps)."""
    msg = update.effective_message
    user = update.effective_user
    if not msg or not user or not msg.text:
        return
    uid = user.id
    if not is_admin(uid) or uid not in _bulk_collect_user_ids:
        return
    bc = context.user_data.get("bulk_collect")
    if not isinstance(bc, dict):
        _bulk_collect_user_ids.discard(uid)
        return
    by_bin = bc.get("by_bin")
    if not isinstance(by_bin, dict):
        _bulk_collect_user_ids.discard(uid)
        context.user_data.pop("bulk_collect", None)
        return
    ...
    errs_mut = bc.get("errors")
    if not isinstance(errs_mut, list):
        errs_mut = []
        bc["errors"] = errs_mut

    try:
        start_ln = max(1, int(bc.get("next_line_no", 1)))
    except (TypeError, ValueError):
        start_ln = 1
    bc["next_line_no"] = merge_bulk_pipe_raw_into(by_bin, errs_mut, raw_lines, start_ln)

    await msg.reply_text(bulk_collect_running_summary_html(bc), parse_mode="HTML")
...
async def cmd_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if uid in _awaiting_custom_topup_user_ids:
        clear_awaiting_custom_topup(uid)
        await update.message.reply_text(
            "Custom top-up cancelled. Open <b>Top-up Balance</b> whenever you’re ready.",
            parse_mode="HTML",
        )
        return
    if uid in _awaiting_tx_link_user_ids:
        ...
        context.user_data.pop("pay_coin", None)
        await update.message.reply_text(
            "Transaction-link step cancelled. Open <b>Top-up Balance</b> whenever you’re ready.",
            parse_mode="HTML",
        )
        return
    if not is_admin(uid):
    ...
    if (
        uid in _admin_stock_wizard_user_ids
        or uid in _bulk_collect_user_ids
        or context.user_data.get("pending_bulk_stock")
        or context.user_data.get("bulk_collect")
    ):
        clear_admin_stock_flow(context, uid)
...
async def cmd_stock_done(
    ...
    if uid in _bulk_collect_user_ids and isinstance(
        context.user_data.get("bulk_collect"), dict
    ):
        bc = context.user_data["bulk_collect"]
        by_bin = bc.get("by_bin")
        if not isinstance(by_bin, dict):
            _bulk_collect_user_ids.discard(uid)
            context.user_data.pop("bulk_collect", None)
            await msg.reply_text("Bulk buffer was corrupt — start over with <code>/stock $price</code>.", parse_mode="HTML")
            return
        tot = sum(len(v) for v in by_bin.values())
        if tot <= 0:
            await msg.reply_text(
                "No valid pipe lines buffered yet. Paste card lines, then <code>/done</code>, "
                "or <code>/cancelstock</code>.",
                parse_mode="HTML",
            )
            return
        price_b = float(bc.get("price") or 0.0)
        errs = list(bc.get("errors") or [])
        context.user_data["pending_bulk_stock"] = {
            "price": price_b,
            "by_bin": {k: list(v) for k, v in by_bin.items()},
            "errors": errs,
        }
        _bulk_collect_user_ids.discard(uid)
        context.user_data.pop("bulk_collect", None)
...
async def cmd_cancelstock(
    ...
    if (
        ...
        and uid not in _bulk_collect_user_ids
    ):
        await update.message.reply_text(
            "Nothing to cancel — no stock wizard, bulk capture, or pile confirmation pending."
        )
        return
    clear_admin_stock_flow(context, uid)
...
async def cmd_stock(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    parsed_open = parse_bulk_stock_opening_message(update.message.text or "")
    if parsed_open is not None:
        price_b, raw_chunk = parsed_open
        clear_admin_stock_flow(context, uid)
        _bulk_collect_user_ids.add(uid)
        by_bin: dict[str, list[str]] = {}
        errs: list[str] = []
        next_ln = merge_bulk_pipe_raw_into(by_bin, errs, list(raw_chunk), 1)
        context.user_data["bulk_collect"] = {
            "price": price_b,
            "by_bin": by_bin,
            "errors": errs,
            "next_line_no": next_ln,
        }
        await update.message.reply_text(
            bulk_collect_running_summary_html(context.user_data["bulk_collect"]),
            parse_mode="HTML",
        )
        return

    args = context.args or []
    if len(args) >= 1 and args[0].lower() == "bin":
        clear_admin_stock_flow(context, uid)
    if len(args) == 0:
    ...
    if args[0].lower() != "bin":
        await update.message.reply_text(
            ...
            "<code>/stock $0.35</code> — optional lines in the same message, then paste the rest "
            "(Telegram splits long lists into multiple bubbles — that’s OK). "
            "When finished, send <code>/done</code>, then tap <b>Live</b> / <b>Unchecked</b>.\n\n"
...
def main() -> None:
    ...
    # are not eaten as BIN / wallet text.
    ...
    app.add_handler(
        MessageHandler(ADMIN_BULK_COLLECT_FILTER, handle_bulk_stock_collect_message)
    )
    app.add_handler(
    ...
    app.add_handler(
        MessageHandler(AWAITING_CUSTOM_TOPUP_FILTER, handle_custom_topup_message)
    )
    app.add_handler(
