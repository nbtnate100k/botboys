...
def shop_cart_list(context: ContextTypes.DEFAULT_TYPE) -> list[dict[str, Any]]:
    raw = context.user_data.get("shop_cart")
    if not isinstance(raw, list):
        raw = []
        context.user_data["shop_cart"] = raw
    return raw
...
def shop_buy_intro_inline() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            ...
            [
                InlineKeyboardButton(
                    "🛒 Cart / checkout", callback_data="cart_go"
                ),
                InlineKeyboardButton("🗑️ Clear cart", callback_data="cart_clr"),
            ],
...
async def checkout_shop_cart(
    ...
    """Return (HTML body, aborted_early)."""
    cart = shop_cart_list(context)
    if not cart:
        return "🛒 <b>Cart is empty.</b>", False
    chunks: list[str] = []

    def _fallback_bin_hint(row: dict[str, Any], sl: str) -> str:
        b0 = str(row.get("bk") or "").strip()[:6]
        if len(b0) == 6 and b0.isdigit():
            return b0
        head = sl.split("-", 1)[0] if "-" in sl else sl
        d = "".join(c for c in head if c.isdigit())[:6]
    ...
    while cart:
        ...
        if not slug:
            cart.pop(0)
            chunks.append("⚠️ Skipped malformed cart row (missing section).")
            continue
        ...
        pcrc_s = ""
        try:
            if it.get("pick_crc") not in (None, "") and it.get("pick_idx") is not None:
                pidx_val = int(it["pick_idx"])
                pcrc_s = str(it["pick_crc"]).strip().lower()
                if pcrc_s and pidx_val >= 0:
                    use_pick = True
        ...
        if use_pick:
            line, sid, err, paid = await execute_shop_sale_pick(
                user, slug, pidx_val, pcrc_s
            )
        else:
            bk_legacy = str(it.get("bk") or "").strip()
            try:
                bank_i = int(it["bank_i"])
                kind_k = int(it["kind_k"])
                level_i = int(it["level_i"])
            except (KeyError, ValueError, TypeError):
                cart.pop(0)
                chunks.append("⚠️ Skipped malformed cart row.")
                continue
            line, sid, err, paid = await execute_shop_sale(
                user,
                slug,
                bk_legacy,
                bank_i,
        ...
        bn_hint = html.escape(_fallback_bin_hint(it, slug))

        if err == "balance":
            u = ensure_user(user.id)
            chunks.append(
                "💳 <b>Insufficient balance</b> — checkout stopped.\n"
                f"You have <b>{fmt_usd(user_shop_balance(u))}</b>; "
                f"this line costs <b>{fmt_usd(paid)}</b>.\n"
                f"<b>{len(cart)}</b> purchase(s) left in cart (top up and tap checkout again)."
            )
            return "\n\n".join(chunks), True
        if err == "section" or err == "stock" or not line or not sid:
            chunks.append(
                f"⚠️ <b>Could not fulfill</b> BIN <code>{bn_hint}</code> "
                f"— listing empty or that line sold. Skipped.\n\n"
                f"<i>{len(cart) - 1} item(s) remain in cart.</i>"
            )
            cart.pop(0)
            continue
        cart.pop(0)
        escaped = html.escape(line)
        _bn = extract_bin_prefix_from_line(line) or "?"
        show_bin = html.escape(_bn[:6])
        chunks.append(
            "✅ <b>Delivered</b> · "
            f"{section_catalog_html_title(sid)}\n"
            f"BIN <code>{show_bin}</code> · paid {fmt_usd(paid)}\n\n"
            f"<code>{escaped}</code>"
        )
    return "\n\n".join(chunks), False
...
def section_catalog_text_and_keyboard(
    ...
    """List every stocked pipe line — BIN plus city/state. Tap adds that line to cart."""
    bank_i = 0
    ...
    text = (
        ...
        f"<b>Balance:</b> {fmt_usd(bal)}\n"
        f"{notes_block}\n"
        "Tap any row to add <b>that exact line</b> to your cart — keep browsing "
        "this page until you checkout."
        f"{more_note}"
    )
...
def try_take_shop_line_unlocked(
    sid: str,
    bucket_key: str,
    bank_i: int,
    kind_k: int,
    ...
    """Remove first matching stocked line (filter-aware). Mutates BIN_SECTIONS only."""
    sec = BIN_SECTIONS.get(sid)
    if not sec:
        return None
    ...
    bk = bucket_key.strip()[:16]
    pick_idx: int | None = None
    for i, ln in enumerate(lines_flat):
        if (extract_bin_prefix_from_line(ln) or "") != bk:
            continue
        if not line_matches_buy_filters(
            ln, bank_i, kind_k, level_i, bank_list, level_list
        ):
            continue
        pick_idx = i
        break
...
async def execute_shop_sale(
    ...
    *,
    ...
    """Return (line, sid, err, paid_price_usd). err empty on success."""
    async with _stock_io_lock:
        ...
        took = try_take_shop_line_unlocked(sid, bucket_key, bank_i, kind_k, level_i)
        if not took:
        ...
        if record_order_row:
            try:
                record_purchase_order(user.id, "shop", sid, bk, price, took)
            except Exception:
...
async def handle_main_reply_menu(
    ...
    if text == BTN_REPLY_BUY:
        await msg.reply_text(
            ...
            "Tap any row → it jumps into your cart; the same page refreshes "
            "so you can keep shopping. Checkout with <b>🛒 Cart / checkout</b>.\n\n"
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith("cac|"):
        ...
        async with _stock_io_lock:
            load_bin_stock()
            sid = slug_to_section_id(slug_raw)
            if not sid or sid not in BIN_SECTIONS:
                await query.answer("Nothing here anymore.", show_alert=True)
                return
            ls = lines_sorted_for_section(BIN_SECTIONS[sid])
            if pick_idx >= len(ls):
                await query.answer("Sold or moved — refresh.", show_alert=True)
                text, kb = section_catalog_text_and_keyboard(
                    slug_raw, user.id, page=max(0, cur_page)
                )
                await edit_safe(query, text, kb)
                return
            line_now = ls[pick_idx]
            if shop_line_pick_crc(line_now) != crc_hex:
                await query.answer("Stale row — refreshed.", show_alert=False)
                text, kb = section_catalog_text_and_keyboard(
                    slug_raw, user.id, page=max(0, cur_page)
                )
                await edit_safe(query, text, kb)
            ...
            dup = False
            for row in cart:
                if not isinstance(row, dict) or row.get("pick_crc") in (None, ""):
                    continue
                try:
                    ridx = int(row.get("pick_idx", -10**9))
                except (TypeError, ValueError):
                    continue
                rs = str(row.get("slug") or "").strip()
                rr = str(row.get("pick_crc") or "").strip().lower()
                if rs == slug_raw and ridx == pick_idx and rr == crc_hex:
                    dup = True
                    break
            if dup:
                await query.answer("Already in cart — pick another.", show_alert=False)
                text, kb = section_catalog_text_and_keyboard(
                    slug_raw, user.id, page=max(0, cur_page)
                )
                await edit_safe(query, text, kb)
                return
            room = MAX_SHOP_CART_ENTRIES - len(cart)
            if room <= 0:
                await query.answer(
                    "Cart full — checkout or clear first.", show_alert=True
                )
                return
            cart.append(
                {
        ...
        await query.answer(f"Added · cart {nc} · keep browsing")
        text, kb = section_catalog_text_and_keyboard(
            slug_raw, user.id, page=max(0, cur_page)
        )
        await edit_safe(query, text, kb)
    ...
    if data == "cart_go":
        ensure_user(user.id)
        body, _aborted = await checkout_shop_cart(context, user)
        await query.answer()
        await query.message.reply_text(
            body,
            reply_markup=main_reply_keyboard(user.id),
    ...
    if data == "cart_clr":
        ...
        cart = shop_cart_list(context)
        cart.clear()
        context.user_data.pop("cart_pick", None)
    ...
    if data == "shop_how_buy":
        ...
        await query.message.reply_text(
            ...
            "• Every line shows <b>BIN + City, ST</b> · tap → adds that card to cart.\n"
            "• The listing stays open so you can add more rows.\n"
            "• <b>Cart / checkout</b> buys in order · balance must cover each line.\n\n"
            "Duplicates of the exact same listing row aren't queued twice.",
            parse_mode="HTML",
        )
        return
    ...
    if data in ("m_cart", "m_buy"):
        await query.message.reply_text(
            "💳 <b>Buy cards</b>\n\n"
            "Pick a pile — each stocked line lists as BIN + City, ST; tap rows for cart.",
            reply_markup=buy_menu_keyboard(),
            parse_mode="HTML",
        )
        return
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    user_lines = (
        ...
        "• <b>Buy cards</b> — BIN pile · BIN + City, ST rows · tap rows → cart\n"
