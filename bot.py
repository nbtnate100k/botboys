...
def cart_qty_pick_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("1", callback_data="cqx|1"),
                InlineKeyboardButton("2", callback_data="cqx|2"),
                InlineKeyboardButton("5", callback_data="cqx|5"),
            ],
            [
                InlineKeyboardButton("10", callback_data="cqx|10"),
                InlineKeyboardButton("20", callback_data="cqx|20"),
                InlineKeyboardButton("50", callback_data="cqx|50"),
            ],
            [[InlineKeyboardButton("❌ Cancel", callback_data="cqxx")]],
        ]
    )
...
async def checkout_shop_cart(
    ...
    while cart:
        ...
        if err == "balance":
            ...
            return "\n\n".join(chunks), True
        if err == "section" or err == "stock" or not line:
            bn = bk or "—"
            chunks.append(
                f"⚠️ <b>Could not fulfill</b> BIN <code>{html.escape(bn)}</code> "
                f"— listing empty or filters no longer match. Skipped.\n\n"
        ...
        chunks.append(
            ...
            f"BIN <code>{html.escape(bk)}</code> · paid {fmt_usd(paid)}\n\n"
            f"<code>{escaped}</code>"
        )
    return "\n\n".join(chunks), False
    ...
    for bk in chunk:
        ...
        rows.append(
            [
                ...
                InlineKeyboardButton(
                    "🛒",
                    callback_data=f"bca:{slug}:{bk}:{bank_i}:{kind_k}:{level_i}",
                ),
            ]
        )
...
async def handle_buy_product(
    ...
    line, sid, err, paid = await execute_shop_sale(
        user, slug, bucket_key, bank_i, kind_k, level_i
    )
    if err == "balance":
        u = ensure_user(user.id)
        ...
        await query.answer(
            ...
            f"This line costs {fmt_usd(paid)}.\n"
    ...
    await query.message.reply_text(
        "✅ <b>Delivered</b> · "
        f"{section_catalog_html_title(sid)}\n"
        f"BIN <code>{html.escape(bucket_key)}</code> · paid {fmt_usd(paid)}\n\n"
        f"<code>{escaped}</code>",
        parse_mode="HTML",
    )
...
async def handle_main_reply_menu(
    ...
    if text == BTN_REPLY_BUY:
        await msg.reply_text(
            "💳 <b>Buy cards</b>\n\n"
            "Open a <b>BIN section</b>, tighten <b>bank / credit–debit / level</b>, "
            "then tap to buy instantly or tap <b>🛒</b> on a row for quantity → cart checkout.\n\n"
            "Use <b>🔍 Filters</b> on the keypad to jump back to section list anytime.",
    ...
    if text == BTN_REPLY_FILTERS:
        await msg.reply_text(
            ...
            "Pick a section, then rotate <b>bank</b>, <b>type</b>, and <b>level</b> "
            "filters at the top of the catalog.",
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith("bca:"):
        ...
        sid = slug_to_section_id(slug.strip())
        if not sid or sid not in bin_sections_all():
            await query.answer("That listing is no longer available.", show_alert=True)
            return
        context.user_data["cart_pick"] = {
            "slug": slug,
            "bk": bucket_key,
            "bank_i": bank_i,
            "kind_k": kind_k,
        ...
        head = html.escape(section_catalog_html_title(sid))
        bk_esc = html.escape(bucket_key.strip()[:16])
        await query.answer("Pick quantity", show_alert=False)
        await query.message.reply_text(
            "🛒 <b>Add to cart</b>\n"
            f"{head}\n"
            f"BIN <code>{bk_esc}</code>\n\n"
            "Tap how many <b>lines</b> to queue (checkout charges your balance "
            "one line at a time).",
            reply_markup=cart_qty_pick_keyboard(),
            parse_mode="HTML",
        )
        return

    if data.startswith("cqx|"):
        ensure_user(user.id)
        try:
            qty = int((data.split("|", 1)[1] if "|" in data else "0").strip())
        except ValueError:
            await query.answer("Invalid quantity.", show_alert=True)
            return
        if qty <= 0:
            await query.answer("Pick a positive number.", show_alert=True)
            return
        cp = context.user_data.get("cart_pick")
        if (
            ...
            or "bank_i" not in cp
            or "kind_k" not in cp
            or "level_i" not in cp
        ):
            await query.answer(
                "Open a catalog listing and tap 🛒 on a BIN row first.",
                show_alert=True,
            )
            return
        try:
            entry = {
                "slug": str(cp["slug"]),
                "bk": str(cp["bk"]).strip(),
                "bank_i": int(cp["bank_i"]),
                "kind_k": int(cp["kind_k"]),
                "level_i": int(cp["level_i"]),
            }
        except (KeyError, ValueError, TypeError):
            context.user_data.pop("cart_pick", None)
            await query.answer("Cart picker expired — try 🛒 again.", show_alert=True)
            return
        cart = shop_cart_list(context)
        room = max(0, MAX_SHOP_CART_ENTRIES - len(cart))
        n_add = min(qty, room, 50)
        if room <= 0:
            await query.answer(
                f"Cart is full ({MAX_SHOP_CART_ENTRIES} lines). Checkout or clear first.",
                show_alert=True,
            )
            return
        for _ in range(n_add):
            cart.append(dict(entry))
        context.user_data.pop("cart_pick", None)
        await query.answer(f"Queued {n_add} line(s) · cart size {len(cart)}.")
        bk_short = html.escape(entry["bk"][:16])
        await query.message.reply_text(
            f"✅ Added <b>{n_add}</b> × BIN <code>{bk_short}</code> · "
            f"cart now <b>{len(cart)}</b> line(s).\n\n"
            "Use <b>🛒 Cart / checkout</b> from Buy to settle when ready.",
            parse_mode="HTML",
        )
    ...
    if data == "shop_how_buy":
        ...
        await query.message.reply_text(
            ...
            "• Pick a <b>BIN section</b> · each line sells once.\n"
            "• Rotate <b>bank</b>, <b>credit / debit</b>, and <b>level slot</b> filters "
            "so the counts match what you want.\n"
            "• Tap a BIN bucket for <b>instant buy</b> (one charged line).\n"
            "• Tap <b>🛒</b> to queue quantity, then "
            "<b>Cart / checkout</b> — the bot fulfills items in order and stops "
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    user_lines = (
        ...
        "• <b>Buy cards</b> — open BIN catalog · filters · buy or cart checkout\n"
        "• <b>Filters</b> — jump to section list\n"
